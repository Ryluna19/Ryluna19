<h1 align="center">Hi, I'm Ryan Santos 👋</h1>

<p align="center">
  Junior Full-Stack Developer focused on building practical web applications.
</p>

<p align="center">
  <a href="https://ryluna19.github.io/PProfile/">Portfolio</a>
  ·
  <a href="https://www.linkedin.com/in/ryan-bulhoes-santos-560b25225/">LinkedIn</a>
</p>

## About me

I build web applications focused on practical solutions and continuous learning.

My main interests are full-stack development, REST APIs, authentication, relational databases, and creating clean, user-friendly interfaces.

Currently seeking internship and junior developer opportunities.

## Main technologies

JavaScript · React · Node.js · Express · PostgreSQL · Python · Flask · Git


## Featured projects

* [Job Application Tracker](https://github.com/Ryluna19/PJobApplicationTracker)
  Full-stack application for managing job applications, with JWT authentication, CRUD operations, dashboard statistics, filters, and PostgreSQL integration.

* [Café Curator](https://github.com/Ryluna19/PCafe_Proximos)
  Web application that uses user location to suggest nearby cafés, with favorites saved through LocalStorage.

* [Flask Library System](https://github.com/Ryluna19/PFlask-Library-System)
  Library management system built with Python, Flask, MySQL, HTML, and CSS.

* [Music Playlist Manager](https://github.com/Ryluna19/PMusicPlaylist)
  Playlist manager with CRUD operations, search, filtering, an admin panel, and a custom audio player.

## Languages used in my public repositories

<p>
  <img
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=Ryluna19&layout=compact&langs_count=6&hide_border=true&theme=transparent"
    alt="Most used languages on GitHub"
  />
</p>

